...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        if results['img_edge_prefix'] is not None:
            edge_filename = osp.join(results['img_edge_prefix'],
                                results['img_info']['filename'])
        else:
            edge_filename = results['img_info']['filename']
        ...
        img_edge_bytes = self.file_client.get(edge_filename)
        img_edge = mmcv.imfrombytes(img_edge_bytes, flag=self.color_type)
        if self.to_float32:
        ...
        results['img_fields'] = ['img','img_edge']
        return results
