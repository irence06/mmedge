...
class Resize:
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img','img_edge']):
            if self.keep_ratio:
            ...
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
            results['img_edge_shape'] = img.shape
            # in case that there is no padding
            results['edge_pad_shape'] = img.shape
            results['edge_scale_factor'] = scale_factor
            results['edge_keep_ratio'] = self.keep_ratio
...
class Pad:
    ...
    def _pad_img(self, results):
        ...
        # for key in results.get('img_fields', ['img_edge']):
        #     if self.pad_to_square:
        #         max_size = max(results[key].shape[:2])
        #         self.size = (max_size, max_size)
        #     if self.size is not None:
        ...
        #             results[key], self.size_divisor, pad_val=pad_val)
        #     results[key] = padded_img
        # results['pad_edge_shape'] = padded_img.shape
        # results['pad_edge_fixed_size'] = self.size
        # results['pad_edge_size_divisor'] = self.size_divisor
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            results[key] = mmcv.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
            key_norm = key+'_norm_cfg'
            results[key_norm] = dict(mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        
        # for key in results.get('img_fields', ['img_edge']):
        #     results[key] = mmcv.imnormalize(results[key], self.mean, self.std,
        #                                     self.to_rgb)
        # results['img_edge_norm_cfg'] = dict(
